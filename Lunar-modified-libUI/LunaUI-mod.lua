...
by Nebula Softworks
modifikasi mod by DarkluaScript and h4000audio
This has been modified by h4000audio and DarkluaScript!
we have copied the library code from sofyworks nebula and modified it a bit
...
Luna.CreateWindow = function(config)
    local window = originalCreateWindow(config)
    
    window.Destroy = function(self)
        for _, obj in pairs(game:GetService("CoreGui"):GetChildren()) do
            if obj:IsA("ScreenGui") and (obj.Name:find("Luna") or obj.Name:find("Window")) then
                obj:Destroy()
                print("[Luna UI] Window destroyed successfully")
                return true
            end
        end
        print("[Luna UI] No window found to destroy")
        return false
    end
